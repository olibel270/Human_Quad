...
#        pozyx.doPositioning(coords, remote_id=remote_id)
       ...
       # if not (coords.x == 0 and coords.y==0): 
         temp_x = 1+random.random() #coords.x/1000
         temp_y = 1-random.random() #coords.y/1000
         pose.pose.position.x = temp_x #(temp_x*np.cos(3.7)+temp_y*np.sin(3.7))
         pose.pose.position.y = temp_y #-(-temp_x*np.cos(3.7)+temp_y*np.sin(3.7))
